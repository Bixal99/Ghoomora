...
import { isCatalogSetupError, getRegion } from "@/lib/data";
...
export default async function RegionPage({ params }: { params: Promise<{ slug: string }> }) {
  let region;
  try {
    region = await getRegion((await params).slug);
  } catch (error) {
  ...
  return (
    <>
      <InnerHeaderShell />
      <main>
        <section className="bg-[#13382f] py-20 text-white">
          <div className="container-shell">
            <p className="eyebrow text-accent">Region guide</p>
            <h1 className="display-title mt-3 text-6xl md:text-8xl">{region.name}</h1>
            <p className="mt-5 max-w-2xl text-lg leading-8 text-white/65">
              {region.blurb ? <>{region.blurb} </> : null}
              Explore {region.destinations.length} places with verified reference coordinates.
            </p>
          </div>
        </section>
