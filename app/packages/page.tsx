...
import { getPackages, getRegions } from "@/lib/data";
    ...
    <>
      ...
      <main>
        ...
        <section className="section-pad"><div className="container-shell grid gap-8 lg:grid-cols-[280px_1fr]">
          ...
          <div><div className="mb-6 flex items-end justify-between"><div><p className="eyebrow text-[#5a7f73]">Sample catalogue</p><h2 className="display-title mt-2 text-5xl">{packages.length} journeys</h2></div></div>{packages.length ? <div className="grid gap-5">{packages.map((pkg) => { const from = Math.min(...pkg.tiers.map((item) => item.pricePerPersonPerDay)); return <Card key={pkg.id} className="p-7"><div className="flex flex-wrap items-start justify-between gap-5"><div className="max-w-2xl"><div className="flex flex-wrap gap-2"><Badge>{pkg.minDays}–{pkg.maxDays} days</Badge>{pkg.stops.some((stop) => stop.destination.requiresLocalTransport) && <Badge className="border-accent bg-[#fff4dc]"><Route size={12} /> Local hire route</Badge>}</div><h3 className="mt-4 text-2xl font-extrabold">{pkg.title}</h3><p className="mt-2 text-xs font-bold text-[#397668]">{pkg.vendor.businessName}</p><p className="mt-4 text-sm leading-7 text-muted-foreground">{pkg.description}</p><div className="mt-5 flex flex-wrap gap-2">{pkg.stops.map((stop) => <span key={stop.destination.slug} className="rounded-full bg-muted px-3 py-1 text-xs">{stop.destination.name}</span>)}</div></div><div className="min-w-44 text-right"><p className="text-xs text-muted-foreground">Tier rate from</p><p className="text-xl font-extrabold">{formatPKR(from)}</p><p className="text-xs text-muted-foreground">per person / day</p><Button asChild className="mt-5"><Link href={"/packages/" + pkg.id}>Configure <ArrowRight size={16} /></Link></Button></div></div></Card>; })}</div> : <Card className="p-10 text-center"><h3 className="text-xl font-extrabold">No exact matches</h3><p className="mt-2 text-muted-foreground">Try removing one filter to see more sample journeys.</p></Card>}</div>
        </div></section>
      </main><SiteFooter />
    </>
  );
