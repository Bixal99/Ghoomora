...
export default async function DashboardPage({ searchParams }: { searchParams: Promise<{ notice?: string }> }) {
  ...
  const db = getDb();
  const stats = db ? await Promise.all([db.vehicle.count({ where: { vendorId: actor.vendor.id } }), db.hotel.count({ where: { vendorId: actor.vendor.id } }), db.package.count({ where: { vendorId: actor.vendor.id } })]) : [0, 0, 0];
  const notice = (await searchParams).notice;
  return <PortalShell><div className="flex flex-wrap items-end justify-between gap-4"><div><p className="eyebrow text-[#5a7f73]">Partner overview</p><h1 className="display-title mt-2 text-6xl">{actor.vendor.businessName}</h1></div><Badge className={actor.vendor.verified ? "border-[#397668] bg-[#dff1e9] text-[#23594c]" : "border-[#d19b48] bg-[#fff2d8] text-[#805b21]"}>{actor.vendor.verified ? <CheckCircle2 size={13} /> : <Clock3 size={13} />}{actor.vendor.verified ? "Verified" : "Awaiting approval"}</Badge></div>{notice && <div className="mt-6 rounded-xl bg-[#dff1e9] p-4 text-sm font-bold text-[#23594c]">{notice}</div>}<div className="mt-8 grid gap-5 md:grid-cols-3">{[{ label: "Vehicles", value: stats[0], icon: Settings2, href: "/fleet" }, { label: "Hotels", value: stats[1], icon: CheckCircle2, href: "/hotels" }, { label: "Packages", value: stats[2], icon: Package, href: "/vendor/packages" }].map((item) => <Card key={item.label} className="p-6"><item.icon className="text-[#397668]" /><p className="mt-8 text-4xl font-extrabold">{item.value}</p><p className="text-sm text-muted-foreground">{item.label}</p><Link href={item.href} className="mt-5 inline-flex items-center gap-1 text-sm font-bold text-[#397668]">Manage <ArrowRight size={15} /></Link></Card>)}</div><Card className="mt-8 p-7"><h2 className="text-xl font-extrabold">Profile capabilities</h2><div className="mt-4 flex flex-wrap gap-2">{actor.vendor.types.map((type) => <Badge key={type}>{type}</Badge>)}</div><p className="mt-5 text-sm leading-7 text-muted-foreground">Public packages are shown only after admin verification. Keep pickup fares and destination-specific local hire separate.</p></Card></PortalShell>;
}
