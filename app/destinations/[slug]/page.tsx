  ...
  return (
    <>
      ...
      <main>
        ...
        <section className="section-pad"><div className="container-shell grid gap-8 lg:grid-cols-[1.2fr_.8fr]"><div><p className="eyebrow text-[#5a7f73]">Know before you go</p><h2 className="display-title mt-3 text-5xl">Mountain context, up front.</h2><div className="mt-8 grid gap-4 sm:grid-cols-3"><Card className="p-5"><Mountain className="text-[#397668]" /><p className="mt-5 text-2xl font-extrabold">{destination.elevationMeters.toLocaleString()}m</p><p className="text-sm text-muted-foreground">Reference elevation</p></Card><Card className="p-5"><CalendarDays className="text-[#397668]" /><p className="mt-5 text-2xl font-extrabold">{months[destination.bestSeasonStart - 1]}–{months[destination.bestSeasonEnd - 1]}</p><p className="text-sm text-muted-foreground">Typical season</p></Card><Card className="p-5"><Route className="text-[#397668]" /><p className="mt-5 text-2xl font-extrabold capitalize">{destination.difficulty}</p><p className="text-sm text-muted-foreground">Planning difficulty</p></Card></div></div><Card className="bg-[#efe3cb] p-7"><AlertTriangle className="text-[#8a5d1d]" /><h3 className="mt-5 text-xl font-extrabold">{destination.requiresLocalTransport ? "Local transport planning required" : "Road conditions still matter"}</h3><p className="mt-3 text-sm leading-7 text-[#6f604a]">{destination.localTransportNote ?? "Live road and weather intelligence arrives in Phase 4. Confirm conditions with your operator before departure."}</p></Card></div></section>
