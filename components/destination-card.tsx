...
export function DestinationCard({ destination, index = 0 }: { destination: DestinationView; index?: number }) {
  return (
    <Card className="group overflow-hidden">
      <div className={"relative h-52 bg-gradient-to-br " + tones[index % tones.length]}>
        <div className="absolute inset-0 opacity-30 [background-image:radial-gradient(circle_at_25%_20%,white_0,transparent_35%)]" />
      ...
      <div className="p-6">
        <div className="flex items-start justify-between gap-4"><div><h3 className="text-xl font-extrabold">{destination.name}</h3><p className="mt-1 flex items-center gap-1 text-sm text-muted-foreground"><Mountain size={15} /> {destination.elevationMeters.toLocaleString()}m · {destination.difficulty}</p></div><Link className="focus-ring grid size-10 shrink-0 place-items-center rounded-full border transition group-hover:bg-primary group-hover:text-white" href={"/destinations/" + destination.slug} aria-label={"View " + destination.name}><ArrowUpRight size={18} /></Link></div>
