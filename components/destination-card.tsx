...
export function DestinationCard({ destination, index = 0 }: { destination: DestinationView; index?: number }) {
  return (
    <Card className="group flex h-full flex-col overflow-hidden transition duration-300 hover:-translate-y-1 hover:shadow-[0_24px_65px_rgba(16,40,32,.13)]">
      <div className={"relative h-52 shrink-0 overflow-hidden bg-gradient-to-br " + tones[index % tones.length]}>
      ...
      <div className="flex flex-1 flex-col p-6">
        ...
        <p className="mt-4 line-clamp-2 flex-1 text-sm leading-6 text-muted-foreground">{destination.description}</p>
      </div>
    </Card>
  );
}
